...
Here, `<template-dir>` means the actual folder name containing the templates.
...
1. Inspect repository structure and active runtime entry points.
2. Read existing docs and identify stale, conflicting, or missing sections.
3. Bootstrap templates:
   - run `./<template-dir>/scripts/bootstrap_docs_from_templates.sh --dry-run`
   - then run `./<template-dir>/scripts/bootstrap_docs_from_templates.sh`
